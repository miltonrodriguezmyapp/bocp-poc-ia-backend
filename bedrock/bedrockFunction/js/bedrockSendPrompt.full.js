...
async function transaction(params) {
  ...
  try {
    ...
    if (params.fileIds && params.fileIds.length > 0) {
      // Caso 1: Se proporcionaron IDs específicos de archivos
      ...
      // Caso 2: No se proporcionaron archivos, buscar los más recientes del usuario
      let fileQuery;
      
      if (params.sessionId) {
        // Primero intentar obtener archivos de la sesión actual
        fileQuery = `
          SELECT 
            bedrockFileId,
            bedrockFileS3Key,
        ...
        // Si no hay sesión, obtener los archivos más recientes del usuario
        fileQuery = `
          SELECT 
            bedrockFileId,
            bedrockFileS3Key,
    ...
    // Extraer los IDs de los archivos que realmente se usaron
